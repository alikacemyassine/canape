import { api, requireAuth, logout, formatPrice, toast, CATEGORIES } from './admin-auth.js';
...
function render() {
    ...
    grid.innerHTML = items.map((p) => `
        <article class="admin-product-card" data-slug="${p.slug}">
            <div class="relative">
                <img src="${p.images?.hero || ''}" alt="${p.name}" onerror="this.style.background='#ece1d3'"/>
            ...
            <div class="p-4">
                ...
                <h3 class="font-headline-md text-lg mb-4" style="font-family:'Playfair Display',serif">${p.name}</h3>
                <div class="flex gap-3 border-t border-outline-variant/30 pt-3">
                    <a href="./article/edit.html?slug=${p.slug}" class="text-on-surface-variant hover:text-primary" title="Modifier">
                        <span class="material-symbols-outlined text-xl">edit</span>
                    </a>
                    <a href="../produits/item/?slug=${p.slug}" target="_blank" class="text-on-surface-variant hover:text-primary" title="Aperçu">
